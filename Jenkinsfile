        ...
        stage('Deploy') {
            steps {
                script {
                        ...
                        publishers: [
                            sshPublisherDesc(
                                ...
                                transfers: [sshTransfer(
                                    execCommand: """
                                        docker pull hanzla9909/distance-converter:${env.BUILD_ID}
                                        docker stop distance-converter-container || true
                                        docker rm distance-converter-container || true
                                        docker run -d --name distance-converter-container -p 80:80 hanzla9909/distance-converter:${env.BUILD_ID}
                                    """
                                )]
                            )
                        ]
                    ...
                    boolean isDeploymentSuccessful = sh(script: 'curl -s -o /dev/null -w "%{http_code}" http://20.2.217.162:80', returnStdout: true).trim() == '200'
